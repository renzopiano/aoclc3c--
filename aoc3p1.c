...
int extractnum(int* puzzle, int i, int j, int w, int h)
{
	int num;
	int* ch;

	num = 0;
	if (!isdigit(*(puzzle+i*(w+1)+j)))
		return num;
	while (isdigit(*(puzzle+i*(w+1)+j)))
		j--;
	j++;
	while (isdigit(*(puzzle+i*(w+1)+j))) {
		num = 10*num + *(puzzle+i*(w+1)+j) - '0';
		ch = puzzle+i*(w+1)+j;
		*ch = '.';
		j++;
	}
...
int sumofparts(int* puzzle)
{
	...
	for (i = 1; i < h-1; i++) {
		for (j = 1; j < w-1; j++) {
			if (!isdigit(*(puzzle+i*(w+1)+j)) && *(puzzle+i*(w+1)+j) != '.') {
				/* symbol */
				enum = extractnum(puzzle, i-1, j-1, w, h);
				if (s + enum >= 0 && s < 0)
					overflows++;
				s = s + enum;
				enum = extractnum(puzzle, i-1, j, w, h);
				if (s + enum >= 0 && s < 0)
					overflows++;
				s = s + enum;
				enum = extractnum(puzzle, i-1, j, w, h);
				...
				s = s + enum;
				enum = extractnum(puzzle, i-1, j, w, h);
				enum = extractnum(puzzle, i, j-1, w, h);
				if (s + enum >= 0 && s < 0)
					overflows++;
				s = s + enum;
				enum = extractnum(puzzle, i-1, j, w, h);
				enum = extractnum(puzzle, i, j, w, h);
				if (s + enum >= 0 && s < 0)
					overflows++;
				s = s + enum;
				enum = extractnum(puzzle, i-1, j, w, h);
...
int main()
{
	/* should print 00083B29 */
	/* format: first and last lines and first and last rows must only contain periods and puzzle must end with newline */
